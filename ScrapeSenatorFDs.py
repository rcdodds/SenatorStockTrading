...
def open_efd_website():
    ...
    chromedriver_path = 'C:\\Users\\rcdodds\\Documents\\chromedriver_win32\\chromedriver.exe'
...
def scrape_headers(driver, connection, cr, from_date):
    ...
    # Add the latest report_id so that the index continues where it left off
    # latest_query = """SELECT DISTINCT report_id FROM header"""
    # latest = int(max([latest_id[0] for latest_id in cr.execute(latest_query).fetchall()]))
    # header['report_id'] = [delta + latest for delta in list(header.index.values)]
    # header.set_index(['report_id'])
...
def scrape_transactions(browser, connect, cursor):
    ...
    links_query = """SELECT report_link FROM header WHERE report_id NOT IN 
                        (SELECT DISTINCT report_id FROM transactions)"""
    links = [link_tuple[0] for link_tuple in cursor.execute(links_query).fetchall()]
    print(links)
...
def main():
    ...
    db_path = 'C:\\Users\\rcdodds\\Documents\\GitHub\\SenatorStockTrading\\SenateEFDs.db'
